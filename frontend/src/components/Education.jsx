...
function Education() {
  ...
  const fetchVideos = async () => {
    try {
      const response = await fetch('https://backend-production-966f2.up.railway.app/api/videos-publicas');
      if (!response.ok) {
  ...
  const fetchArchivedVideos = async () => {
    try {
      const response = await fetch('https://backend-production-966f2.up.railway.app/api/videos/archivados/todos', {
        headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = editingId 
        ? 'https://backend-production-966f2.up.railway.app/api/videos/${editingId}'
        : 'https://backend-production-966f2.up.railway.app/api/videos';
  ...
  const handleDelete = async (id) => {
    if (window.confirm('¿Está seguro de que quiere archivar este video?')) {
      try {
        const response = await fetch(`https://backend-production-966f2.up.railway.app/api/videos/${id}`, {
          method: 'DELETE',
          credentials: 'include'
        });
  ...
  const handleRestore = async (id) => {
    if (window.confirm('¿Está seguro de que quiere restaurar este video como publicado?')) {
      try {
        const response = await fetch(`https://backend-production-966f2.up.railway.app/api/videos/${id}/restore`, {
          method: 'PATCH',
          headers: {
            'Content-Type': 'application/json'
          },
  ...
  const handlePermanentDelete = async (id) => {
    if (window.confirm('⚠️ ¿Está seguro de que quiere ELIMINAR PERMANENTEMENTE este video de la base de datos? Esta acción no se puede deshacer.')) {
      try {
        const response = await fetch(`https://backend-production-966f2.up.railway.app/api/videos/${id}/permanent`, {
          method: 'DELETE',
          credentials: 'include'
        });
